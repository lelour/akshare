...
def fx_spot_quote() -> pd.DataFrame:
    ...
    # 使用代理
    res = post(url=FX_SPOT_URL, data=payload, headers=SHORT_HEADERS)
...
def fx_swap_quote() -> pd.DataFrame:
    ...
    # 使用代理
    res = post(url=FX_SWAP_URL, data=payload, headers=SHORT_HEADERS)
...
def fx_pair_quote() -> pd.DataFrame:
    ...
    # 使用代理
    res = post(url=FX_PAIR_URL, data=payload, headers=SHORT_HEADERS)
