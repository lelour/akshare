...
def _get_zh_bond_hs_cov_page_count() -> int:
    ...
    r = get(zh_sina_bond_hs_cov_count_url, params=params)
...
def bond_zh_hs_cov_spot() -> pd.DataFrame:
    ...
    for page in tqdm(range(1, page_count + 1), leave=False):
        ...
        res = get(zh_sina_bond_hs_cov_url, params=zh_sina_bond_hs_payload_copy)
...
def bond_zh_hs_cov_daily(symbol: str = "sh010107") -> pd.DataFrame:
    ...
    # 使用代理
    r = get(zh_sina_bond_hs_cov_hist_url.format(symbol, datetime.datetime.now().strftime("%Y_%m_%d")))
